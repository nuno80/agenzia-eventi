...
export default function EventDashboard({
  ...
  const participantStatus = getStatusFromRate(insights.occupancyRate, {
    warn: 70,
    error: 90,
  });
  const sessionStatus = getStatusFromRate(
    100 - insights.sessionCompletionRate,
    { warn: 20, error: 50 }
  );
  const speakerStatus = getStatusFromRate(
    100 - insights.speakerConfirmationRate,
    { warn: 20, error: 50 }
  );
  const budgetStatus = getStatusFromRate(insights.budgetUtilizationRate, {
    warn: 70,
    error: 90,
  });
              ...
              {speakers.slice(0, 5).map((speaker) => (
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-muted-foreground">
                      {speaker.sessions_count} sessioni •{" "}
                      {formatCurrency(speaker.total_travel_reimbursements)}{" "}
                      rimborsi
                    </div>
                  </div>
                  <Badge variant="outline">
                    {speaker.confirmed_sessions === speaker.sessions_count
                      ? "Completo"
                      : "In corso"}
                  </Badge>
                </div>
