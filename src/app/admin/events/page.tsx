...
import { Calendar, Clock, DollarSign, PlusCircle, Users } from "lucide-react";
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {events.reduce(
                (sum, event) => sum + (event.participant_count || 0),
                0
              )}
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {events.reduce(
                (sum, event) => sum + (event.session_count || 0),
                0
              )}
            </div>
            <p className="text-xs text-muted-foreground">
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="mt-2 flex items-center gap-4 text-sm text-gray-500">
                      ...
                      {event.price && event.price > 0 && (
                        <span className="flex items-center gap-1">
                          <DollarSign className="h-4 w-4" />€{event.price}
                        </span>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <Button asChild variant="outline" size="sm">
                      <Link href={`/admin/events/${event.id}`}>Dettagli</Link>
                    </Button>
                  </div>
                </div>
              ))}
