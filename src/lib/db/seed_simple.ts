...
function createRequiredTables(db: any) {
  ...
  try {
    ...
    `);
    console.log('✅ event_participant table created');
  } catch (error) {
    console.error('Error creating event_participant table:', error);
  }

  // Create event_speaker table
  try {
    db.run(`
    ...
    `);
    console.log('✅ event_speaker table created');
  } catch (error) {
    console.error('Error creating event_speaker table:', error);
  }
  ...
  try {
    ...
    console.error('Error creating backup_jobs table:', error);
  }
}
...
async function seedDatabase() {
  ...
  try {
    ...
    const participantMappings = [
      ...
      { userId: 'user_9_participant5', eventId: 1, status: 'CHECKED_OUT', email: 'lorenzo.conti@participant.test' },
      { userId: 'user_10_participant6', eventId: 1, status: 'WAITLISTED', email: 'chiara.romano@participant.test' },
      
      // Webinar Marketing (event_id: 2)
      { userId: 'user_3_participant1', eventId: 2, status: 'REGISTERED', email: 'marco.rossi@participant.test' },
      { userId: 'user_4_participant2', eventId: 2, status: 'REGISTERED', email: 'giulia.bianchi@participant.test' },
      { userId: 'user_5_participant3', eventId: 2, status: 'CHECKED_IN', email: 'alessandro.verdi@participant.test' },
      { userId: 'user_8_participant4', eventId: 2, status: 'REGISTERED', email: 'sara.martini@participant.test' },
